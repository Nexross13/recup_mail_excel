...
def lire_colonne_f(fichier_excel, fichier_sortie, colonne):
    try:
        # Lecture du fichier Excel
        donnees = pd.read_excel(fichier_excel, engine='openpyxl')
        
        # Vérification si la colonne F existe
        if colonne in donnees.columns or donnees.shape[1] >= (ord('F') - 64):
            ...
            print(f"La colonne {colonne} n'existe pas dans le fichier fourni.")
...
fichier_excel = "merch.xlsx"  # Remplacez par le chemin de votre fichier Excel
fichier_sortie = "emails_uniques.txt"  # Nom du fichier de sortie
colonne = 'F'
lire_colonne_f(fichier_excel, fichier_sortie, colonne)